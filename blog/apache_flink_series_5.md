title: Building Applications with Apache Flink (Part 3): Complex Event Processing with Apache Flink
...
A warning is always generated by a certain pattern, so we create an interface ``IWarningPattern``. The actual patterns for will be defined with the Apache Flink [Pattern API]. 

Once a pattern has been matched, Apache Flink emits a ``Map<String, TEventType>`` to the environment. The map contains the names and events of the match. So implementations 
of the ``IWarningPattern`` also need to define how to convert pattern matches into a certain warning. And finally to simplify reflection when building the Apache Flink stream 
processing pipeline, the ``IWarningPattern`` also returns the type of the warning. 
